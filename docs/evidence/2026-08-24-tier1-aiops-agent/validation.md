...
- Residual risk：replacement artifact 尚未上傳或部署；舊 exactly-one marker 必須保留且不得用既有 release 直接重跑。新版本仍需先完成 zero-model gate，再另行核准 bounded Nova Lite invocation；在有效分析、人工批准與受控 recovery action完成前，不宣稱 AIOps AWS Demo 完成。
