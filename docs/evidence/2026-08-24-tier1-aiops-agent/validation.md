...
- Residual risk：release 已部署且真實模型邊界能安全拒絕不合 schema 的輸出，但尚無有效 AIOps 分析、人工批准或受控 recovery action；單次 marker 必須保留且不得直接重跑。下一步先以本機嚴格 TDD 增加安全的 failure classification 或 bounded normalization，重建 release 後才評估新的 bounded invocation。
