...
- Targeted `29 passed`；Tier 3 affected `36 passed`；Backend `414 passed, 8 skipped`；Frontend `94 passed`。
- GitHub workflow與 CloudFormation YAML parse通過；template含 6 個 resources與兩個獨立 SSM Documents。
- Sensitivity 暫時移除 migration 後 legacy readiness marker，ordering contract精確失敗；還原後單測通過，mutation未保留。
- 本機 ARM64 build、non-root `10001:10001`、固定 release assets、migration與 container `live`／`ready` health通過。
