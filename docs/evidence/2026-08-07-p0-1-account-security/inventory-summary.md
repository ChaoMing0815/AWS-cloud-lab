...
| 項目 | 舊帳號驗證結果 |
| --- | --- |
| AWS Budget | `My Zero-Spend Budget` 運作狀態正常；每月預算 `US$1.00`；目前支出 `US$0.00`；提醒閾值已確認 |
| Root MFA | 已啟用；IAM 儀表板安全建議為 0 |
| Root Access Key | 沒有作用中的 Root Access Key |
| AWS Organizations | 已啟用，登入帳號為 management account |
| CloudTrail Event history | 可查詢最近管理事件；篩選結果包含 Root 的 `ConsoleLogin`，事件來源為 `signin.amazonaws.com` |

> 以上相關 Console 截圖已於 2026-08-10 清除，避免與新帳號現況混淆；此表僅供事故歷史追溯。
