...
| 群組與初始權限 | `AWSFinalProjectDevelopers` 有 1 位成員；連接 `ReadOnlyAccess`、`IAMUserChangePassword`、`AWSBillingReadOnlyAccess` | [`phase0-ming-dev-group-policies.png`](../../screenshots/phase0-ming-dev-group-policies.png) |
| Billing 有效權限 | `ming-dev` 可開啟 2026 年 8 月帳單；Billing 唯讀由群組的 `AWSBillingReadOnlyAccess` 提供 | [`phase0-ming-dev-group-policies.png`](../../screenshots/phase0-ming-dev-group-policies.png)／[`phase0-ming-dev-billing-zero.png`](../../screenshots/phase0-ming-dev-billing-zero.png) |
...
- `phase0-ming-dev-group-policies.png` 已使用 2026-08-10 20:51 的更新畫面取代同日較早只顯示 2 個 policies 的版本。
- 更新畫面顯示「許可政策 (3)」並完整列出 `AWSBillingReadOnlyAccess`、`IAMUserChangePassword`、`ReadOnlyAccess`；群組 ARN 已遮蔽。
