...
| 2026-09-02 | 寵物視覺 v1.1.1 production release | PR #74以UI-only patch將寵物重塑為果凍史萊姆並遞增`Release v1.1.1`；使用者核准exact main `4fb06d0…`、previous digest `sha256:14d8e0f…`的`digest-release`。Main CI run `33582692830`與release run `33583003508`通過Frontend／Backend、approval、OIDC、ARM64 immutable image、digest fence、Trivy、bounded SSM與metrics artifact | Active Web更新為`sha256:ad0ee896…`並維持`async`；SSM `Success`／response `0`，strict TLS首頁／live／ready皆`200`。Production Browser確認`Release v1.1.1`、果凍本體／裙邊／直接表情與對話框；舊機器人面板／分離腿不存在。Rules retrieval、Backend、schema、Publisher、Worker、IAM、RAG與AWS資源均未變 | [寵物視覺 v1.1.1 production驗證](evidence/2026-09-02-pet-visual-v1-1-1-production-release/validation.md) |
| 2026-09-04 | Release v1.1.2 Web production release | PR #77合併星火規則與展示UI patch；exact source `09dc09a…`的main CI run `33844447220`與Tier 3 release run `33844595314`成功 | Active Web更新為`sha256:c3e6c215…`、runtime維持`async`，玩家可見`Release v1.1.2`。後續已合併的v1.1.3尚未部署；Publisher與兩台Worker未由此release變更 | [目前狀態](handoffs/CURRENT.md) |
| 2026-09-05 | Nova Lite ToolUse Worker-only hotfix production rollout | PR #80登記治理邊界、PR #81合併hotfix至exact main `3246f2a…`；main CI run `33896333755`成功。Worker artifact run `33897173518`通過production approval、ARM64 immutable build／push、exact-digest Trivy與manifest；使用者再透過Systems Manager逐台更新兩台private Worker | 兩台Worker均為digest `sha256:1655de7a…`、max tokens `3000`、service enabled／active、container running、restart `0`、mode `async`、registry auth absent。玩家測試Round 01第一次`applied`，Round 02則三次後`failed/SCHEMA_INVALID`；safe diagnostics為`round_narrative_bounds`與`round_action_consequence_bounds`，顯示3000輸出budget未和600／240字元validator對齊。Web仍是v1.1.2／`sha256:c3e6c215…`，Publisher不變；此hotfix不得標為已解決。Rollback為`sha256:2d5d5866…`＋tokens `800` | [ToolUse hotfix驗證](evidence/2026-09-05-bedrock-tooluse-hotfix/validation.md) |
