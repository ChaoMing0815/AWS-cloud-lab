...
| 2026-08-24 | Tier 1 Batch 11B Agent install／首次 runtime 設定 | 使用者透過 SSM Distributor 在既有 EC2 安裝 CloudWatch Agent；首次啟用安全 JSONL sink 時重啟 application，loopback 檢查失敗即停止並嘗試 rollback；未執行 AWS CLI、S3 或 Bedrock | package 安裝成功；設定階段回報 `curl (7)`、`rollback=attempted`、`configure_result=7`。Agent 尚未啟用、log delivery 尚未驗證；application 與 rollback 最終狀態待唯讀確認 | [Batch 11B 執行摘要](evidence/2026-08-24-tier1-runtime-observability/validation.md) |
