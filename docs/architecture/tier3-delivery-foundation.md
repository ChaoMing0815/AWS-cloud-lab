...
target unit以唯一的systemd設定`Environment=CO_STORY_RESOLUTION_MODE=sync`定義bridge同步模式；Docker CLI必須在同一個`ExecStart`以`--env CO_STORY_RESOLUTION_MODE=${CO_STORY_RESOLUTION_MODE}`傳遞此allowlisted、非秘密值。這是兩層不同的邊界：systemd Environment不會隱式成為container環境，因此禁止只依賴`runtime.env`或在Docker命令硬編第二份mode。此規則不改變candidate的顯式literal sync，亦不改變digest-release或schema-activation路徑。
