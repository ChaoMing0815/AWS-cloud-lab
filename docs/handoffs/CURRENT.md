...
- AWS active release：`tier1-20260825-38296e7`。
...
下一個執行起點：保留 EC2 上舊 exactly-one marker；等待使用者另行核准 forced-tool 版本的一次 bounded Nova Lite invocation。有效分析必須先回報並取得人工批准，才執行 allowlisted action；完成後更新 Tier 1 gate並開始 Tier 2 repo-local queue／job／idempotency 設計。Agent 仍禁止執行 AWS CLI、S3 讀取或 Bedrock 呼叫。
...
- Bounded AIOps Agent replacement 已部署並通過 zero-model gate，舊呼叫 marker仍保留；尚無 forced-tool 版本的有效真實分析、人工批准與受控 recovery action，目前不得宣稱 AIOps AWS Demo 已完成或直接重跑。
