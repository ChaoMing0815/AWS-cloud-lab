...
- Internal staging release 已更新為 `tier0-20260818-7b89e60`：Batch 6A.1 只讀取 exact archive／checksum，驗證 checksum 後從 archive 取出 update installer；未重新取得 master secret。`co-story.service` 與 `co-story-nginx-staging.service` 均為 active，`/opt/co-story/current` 指向該 release，`http://127.0.0.1:8080/api/v1/ready` 回傳 HTTP `200`。
