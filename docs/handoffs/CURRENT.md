...
- Git checkpoint：Batch 6A least-privilege release-update tip `7b89e60`；Batch 5A AWS validation manifest `6ed5c0c`
- 最後全綠功能基準：`7b89e60`（Direct EC2 IP HTTPS／ACME renewal／不重新取得 bootstrap credential 的 release update；尚未部署）
- Regression：Backend `300 passed, 8 skipped`；Frontend `80 passed`（2026-08-18 重跑）
...
- Batch 6A 本機 R3 gate 已完成：ACME-only HTTP bootstrap、固定 IP HTTPS Nginx、Certbot `5.4.0`、12 小時 renewal timer、production env 與驗證後 rollback assets 全綠；另在 preflight 發現原 installer 會重新要求已撤除的 master-secret bootstrap 權限，已以 Red→Green 補上只沿用既有 protected DB environment 的 update installer，代表性 master-secret mutation 被攔截。修正版 release `tier0-20260818-7b89e60` checksum `OK` 且已由使用者上傳至既有 private artifact prefix；尚未部署。因 Batch 6A 明文不含 AWS CLI，exact-object S3 read＋internal activation 已另定義為待核准 Batch 6A.1。
...
→ 核准 Batch 6A.1 後，以 SSM exact-object S3 read 安裝全綠 release 並驗證 internal staging
