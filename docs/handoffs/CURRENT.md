...
下一個開發起點：先以 Console 完成 deployed log-write policy 的 Access Analyzer／attachment property 驗證，再提出 Batch 11B change envelope；11B 才允許透過 SSM Distributor 安裝 CloudWatch Agent、套用固定 config、啟用安全 JSONL sink 與驗證 log delivery。未核准 11B 前不得執行 AWS CLI、Run Command、deploy、S3 讀取或 Bedrock 呼叫。
